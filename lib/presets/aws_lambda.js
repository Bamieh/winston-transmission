...
function awsLambdaPreset(logger, event, context, customContext = () => {}) {
  ...
  if (sentryLogger) {
    ...
    sentryLogger.raven.mergeContext(customContext(event))
  }
}
...
function addUserContext(logger, event, context) {
  ...
  if (!isNil(identity)) {
    ravenContext = Object.assign({}, ravenContext, {
      user: {
        id: identity.cognitoIdentityId,
        username: identity.user,
        ip_address: ravenContext.ip_address || identity.sourceIp,
        user_agent: ravenContext.user_agent || identity.userAgent
      }
    })
  }

  logger.raven.mergeContext(ravenContext)
}
